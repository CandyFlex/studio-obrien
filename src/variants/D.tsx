...
/* ─── SCATTERED LEAVES ─── */
function ScatteredLeaves({ phase }: { phase: "closed" | "crack" | "partial" | "open" }) {
...
/* ─── FOLIAGE CURTAIN (MULTI-STAGE THEATRICAL) ─── */
function FoliageCurtain({ onOpen }: { onOpen: () => void }) {
  const [phase, setPhase] = useState<"closed" | "crack" | "partial" | "open">("closed");

  useEffect(() => {
    // Phase 1: "crack" - 1500ms after mount
    const t1 = setTimeout(() => setPhase("crack"), 1500);

    // Phase 2: "partial" - 3000ms after mount
    const t2 = setTimeout(() => setPhase("partial"), 3000);

    // Phase 3: "open" - 4500ms after mount
    const t3 = setTimeout(() => {
      setPhase("open");
      onOpen();
    }, 4500);
  ...
  const clipL =
    "polygon(0% 0%, 55% 0%, 52% 8%, 56% 16%, 51% 24%, 54% 32%, 50% 40%, 53% 48%, 49% 56%, 52% 64%, 48% 72%, 51% 80%, 47% 88%, 50% 100%, 0% 100%)";
  const clipR =
    "polygon(100% 0%, 45% 0%, 48% 8%, 44% 16%, 49% 24%, 46% 32%, 50% 40%, 47% 48%, 51% 56%, 48% 64%, 52% 72%, 49% 80%, 53% 88%, 50% 100%, 100% 100%)";
  ...
  // Determine panel movement based on phase
  const getLeftPanelX = () => {
    switch (phase) {
      case "crack":
        return "-3%";
  ...
  // Get spring stiffness based on phase
  const getSpringConfig = () => {
    switch (phase) {
      case "crack":
        return { stiffness: 80, damping: 25, mass: 1 };
      case "partial":
        return { stiffness: 50, damping: 20, mass: 1.2 };
      case "open":
        return { stiffness: 40, damping: 18, mass: 1.5 };
      default:
        return { stiffness: 60, damping: 18, mass: 1.2 };
    }
  };
  ...
  return (
    <>
      {!hidden && <ScatteredLeaves phase={phase} />}

      {/* Ambient golden glow behind curtain (pulsing) */}
      <motion.div
        className="fixed top-0 bottom-0 left-1/2 z-[50] pointer-events-none"
        style={{
          width: "100%",
          transform: "translateX(-50%)",
          background: `radial-gradient(ellipse 40% 80% at 50% 50%, ${C.gold}15, transparent)`,
          animation: hidden ? "none" : "hg-pulse 3s ease-in-out infinite",
        }}
        animate={{ opacity: hidden ? 0 : 1 }}
        transition={{ duration: 0.8 }}
      />

      {/* Light beam effect - crack and partial phases */}
      <motion.div
        ...
        animate={{
          width: phase === "crack" ? "20px" : phase === "partial" ? "60px" : "2px",
          opacity: phase === "crack" ? 1 : phase === "partial" ? 0.6 : 0,
        }}
      ...
      >
        <div
          ...
          style={{
            background: `linear-gradient(90deg, transparent, ${C.brightGold}80, ${C.paleGold}ff, ${C.brightGold}80, transparent)`,
            boxShadow: `0 0 40px ${C.gold}80, 0 0 80px ${C.gold}40, inset 0 0 40px ${C.brightGold}60`,
          }}
        />
      </motion.div>

      {/* Center gold glow seam */}
      <motion.div
        className="fixed top-0 bottom-0 z-[52] pointer-events-none"
        style={{
          left: "50%",
          width: 4,
          marginLeft: -2,
          background: `linear-gradient(180deg, transparent, ${C.gold}40, ${C.brightGold}80, ${C.gold}40, transparent)`,
          boxShadow: `0 0 30px ${C.gold}60, 0 0 60px ${C.gold}30`,
        }}
        animate={{ opacity: phase === "closed" ? 1 : phase === "crack" ? 0.6 : 0 }}
        transition={{ duration: 1 }}
      />

      {/* Left panel */}
      <motion.div
        style={{ ...shared, left: 0, right: "50%", clipPath: clipL, pointerEvents: hidden ? "none" : "auto" }}
        animate={{ x: getLeftPanelX(), opacity: hidden ? 0 : 1 }}
        transition={{
          type: "spring",
          ...getSpringConfig(),
      ...
      >
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${C.deepForest}, ${C.forest}90, ${C.emerald}60)` }} />
      </motion.div>

      {/* Right panel */}
      <motion.div
        style={{ ...shared, left: "50%", right: 0, clipPath: clipR, pointerEvents: hidden ? "none" : "auto" }}
        animate={{ x: getRightPanelX(), opacity: hidden ? 0 : 1 }}
        transition={{
          type: "spring",
          ...getSpringConfig(),
          opacity: { duration: 0.3, delay: 0.5 },
      ...
      >
        <div className="absolute inset-0" style={{ background: `linear-gradient(225deg, ${C.deepForest}, ${C.forest}90, ${C.emerald}60)` }} />
      </motion.div>
    </>
  );
}
...
export default function App() {
  ...
  // Track curtain phase for hero content timing
  useEffect(() => {
    const t1 = setTimeout(() => setPhase("crack"), 1500);
    const t2 = setTimeout(() => setPhase("partial"), 3000);
      ...
      >
        ...
        >
          ...
          <motion.div
            ...
            transition={{
              duration: phase === "open" ? 1 : 0.8,
              delay: phase === "partial" ? 0 : phase === "open" ? 0.2 : 0,
            }}
          ...
          <motion.p
            ...
            transition={{
              duration: phase === "open" ? 1 : 0.8,
              delay: phase === "partial" ? 0 : phase === "open" ? 0.4 : 0,
            }}
          ...
          {/* Main Title */}
          <motion.h1
            initial={{ opacity: 0.1, y: 20, scale: 0.98 }}
            animate={
              phase === "crack"
                ? { opacity: 0.1, y: 0, scale: 0.98 }
                : phase === "partial"
                ? { opacity: 0.5, y: 0, scale: 0.99 }
                : phase === "open"
          ...
          <motion.div
            ...
            transition={{
              duration: phase === "open" ? 1 : 0.8,
              delay: phase === "partial" ? 0.2 : phase === "open" ? 0.8 : 0,
            }}
          ...
          <motion.p
            ...
            transition={{ duration: 0.8, delay: phase === "open" ? 1 : 0 }}
          ...
          <motion.a
            ...
            transition={{ duration: 0.8, delay: phase === "open" ? 1.2 : 0 }}
