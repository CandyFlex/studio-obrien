...
/* ─── CURTAIN PANELS (click-activated golden crest) ─── */
function FoliageCurtain({ onOpen }: { onOpen: () => void }) {
  const [phase, setPhase] = useState<"closed" | "opening" | "open">("closed");
  const [crestClicked, setCrestClicked] = useState(false);
  const triggered = useRef(false);

  const handleCrestClick = () => {
    if (triggered.current) return;
    triggered.current = true;
    setCrestClicked(true);

    // After crest animation (0.4s), trigger the burst
    setTimeout(() => {
      setPhase("opening");
      document.body.style.overflow = "";
    }, 300);

    // Total time to fully open
    setTimeout(() => {
      setPhase("open");
      onOpen();
    }, 1800);
  ...
  /* Snappy spring config for burst effect */
  const burstSpring = {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={{
          ...burstSpring,
          opacity: { duration: 0.01, delay: gone ? 0 : 99 },
        }}
      >
        <img
      ...
      <motion.div
        ...
        animate={{ opacity: gone || isOpen ? 0 : 1 }}
      ...
      >
        <motion.button
          ...
          className="relative pointer-events-auto w-24 h-24 rounded-full flex items-center justify-center cursor-pointer outline-none border-2"
          style={{
            borderColor: C.brightGold,
            background: `linear-gradient(135deg, ${C.brightGold}, ${C.gold})`,
          ...
          whileHover={!crestClicked ? { scale: 1.15, boxShadow: `0 0 50px ${C.brightGold}80` } : {}}
        >
          <span
